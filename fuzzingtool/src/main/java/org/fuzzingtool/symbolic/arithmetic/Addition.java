...
public class Addition extends SymbolicNode {
    public Addition(LanguageSemantic s, SymbolicNode a, SymbolicNode b) throws SymbolicException.WrongParameterSize {
        ...
        /*if (a.type == b.type && (a.type == ExpressionType.INT || a.type == ExpressionType.REAL)) {
            this.type = a.type;
            addChildren(2, a, b);
        } else if (a.type == ExpressionType.INT && b.type == ExpressionType.REAL || a.type == ExpressionType.REAL && b.type == ExpressionType.INT) {
            this.type = ExpressionType.REAL;
            addChildren(2, a, b);
        } else if (a.type == ExpressionType.STRING || b.type == ExpressionType.STRING) {
    ...
    /**
     * Addition operator as in <a href="https://tc39.es/ecma262/2020/#sec-addition-operator-plus">ECMAScript® 2020 Language Specification</a>
     *
    ...
    public Pair<Expr, ExpressionType> toZ3ExprJS(Context ctx) throws SymbolicException.NotImplemented, SymbolicException.UndecidableExpression {
        ...
        if (a.getRight() == ExpressionType.STRING || b.getRight() == ExpressionType.STRING) {
            ...
            return Pair.create(ctx.mkConcat((SeqExpr) a_string.getLeft(), (SeqExpr) b_string.getLeft()), ExpressionType.STRING);
        }
        ...
        if (checkTypeAll(ExpressionType.BIGINT, a_numeric, b_numeric)) {
            return Pair.create(ctx.mkAdd((ArithExpr) a_numeric.getLeft(), (ArithExpr) b_numeric.getLeft()), ExpressionType.BIGINT);
        }
        if (checkTypeAll(ExpressionType.NUMBER_INTEGER, a_numeric, b_numeric)) {
            return Pair.create(ctx.mkAdd((ArithExpr) a_numeric.getLeft(), (ArithExpr) b_numeric.getLeft()), ExpressionType.NUMBER_INTEGER);
        }
        return Pair.create(ctx.mkAdd((ArithExpr) a_numeric.getLeft(), (ArithExpr) b_numeric.getLeft()), ExpressionType.NUMBER_REAL);
    }
}
