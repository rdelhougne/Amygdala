...
public class StrictEqual extends SymbolicNode {
    public StrictEqual(LanguageSemantic s, SymbolicNode a, SymbolicNode b) throws SymbolicException.WrongParameterSize {
    ...
    /**
     * Performs an abstract strict equality comparison as in <a href="https://tc39.es/ecma262/2020/#sec-abstract-equality-comparison">ECMAScript® 2020 Language Specification</a>
     *
     ...
     */
    public static Pair<Expr, ExpressionType> abstractStrictEqualityComparisonZ3JS(Context ctx, Pair<Expr, ExpressionType> x, Pair<Expr, ExpressionType> y) throws SymbolicException.UndecidableExpression, SymbolicException.NotImplemented {
        if (!typeEquality(x, y)) {
    ...
    public static boolean typeEquality(Pair<Expr, ExpressionType> x, Pair<Expr, ExpressionType> y) {
        switch (x.getRight()) {
            ...
            default:
                return y.getRight() == ExpressionType.NUMBER_INTEGER || y.getRight() == ExpressionType.NUMBER_REAL || y.getRight() == ExpressionType.NUMBER_NAN || y.getRight() == ExpressionType.NUMBER_POS_INFINITY || y.getRight() == ExpressionType.NUMBER_NEG_INFINITY;
        }
    }

    //https://tc39.es/ecma262/2020/#sec-numeric-types-number-equal
    public static Pair<Expr, ExpressionType> numericEquality(Context ctx, Pair<Expr, ExpressionType> x, Pair<Expr, ExpressionType> y) throws SymbolicException.UndecidableExpression {
        if (checkTypeContains(ExpressionType.NUMBER_NAN, x, y)) {
            return Pair.create(ctx.mkFalse(), ExpressionType.BOOLEAN);
        }
        if (checkTypeAll(ExpressionType.NUMBER_POS_INFINITY, x, y) || checkTypeAll(ExpressionType.NUMBER_NEG_INFINITY, x, y)) {
            return Pair.create(ctx.mkTrue(), ExpressionType.BOOLEAN);
        }
        if (checkTypeContains(ExpressionType.NUMBER_POS_INFINITY, x, y) || checkTypeContains(ExpressionType.NUMBER_NEG_INFINITY, x, y)) {
    ...
    /**
     * Performs a comparison between non-numeric literals <a href="https://tc39.es/ecma262/2020/#sec-samevaluenonnumeric">ECMAScript® 2020 Language Specification</a>
     *
     * @param ctx Z3-Context
     * @param x left-side Parameter
     * @param y right-side Parameter
