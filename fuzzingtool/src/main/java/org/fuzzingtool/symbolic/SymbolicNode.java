...
public abstract class SymbolicNode {
     ...
     * @return A Pair: first slot is the complete symbolic datastructure, second slot is the type of the expression, dependant on the language semantic of the node.
     */
    public final Pair<Expr, ExpressionType> toZ3Expr(Context ctx) throws SymbolicException.NotImplemented, SymbolicException.UndecidableExpression {
        if (this.languageSemantic == LanguageSemantic.JAVASCRIPT) {
    ...
    public abstract Pair<Expr, ExpressionType> toZ3ExprJS(Context ctx) throws SymbolicException.UndecidableExpression, SymbolicException.NotImplemented;
    ...
    public static final Pair<Expr, ExpressionType> tryCastZ3JS(Context ctx, Pair<Expr, ExpressionType> expression, ExpressionType goal) throws SymbolicException.UndecidableExpression { //TODO
        ...
        throw new SymbolicException.UndecidableExpression("Z3", "Cannot cast expression of type '" + expression.getRight().toString() + "' to '" + goal.toString() + "'.");
    }
    ...
    public static final Pair<Expr, ExpressionType> toNumericZ3JS(Context ctx, Pair<Expr, ExpressionType> expression) throws SymbolicException.UndecidableExpression { //TODO
        ...
        if (expression.getRight() == ExpressionType.BOOLEAN) {
            if (ENHANCED_CASTING) {
                ...
                throw new SymbolicException.UndecidableExpression("Z3", "Cannot cast expression of type '" + expression.getRight().toString() + "' to a Number type.");
            } else {
                throw new SymbolicException.UndecidableExpression("Z3", "Cannot cast expression of type '" + expression.getRight().toString() + "' to a Number type.");
            }
        }
    ...
    public static final Pair<Expr, ExpressionType> toBooleanZ3JS(Context ctx, Pair<Expr, ExpressionType> expression) throws SymbolicException.NotImplemented {
        switch (expression.getRight()) {
            case BOOLEAN:
                return expression;
            case STRING:
                return Pair.create(ctx.mkNot(ctx.mkEq(ctx.mkLength((SeqExpr) expression.getLeft()), ctx.mkInt(0))), ExpressionType.BOOLEAN);
            case BIGINT:
            ...
            case SYMBOL:
                return Pair.create(ctx.mkTrue(), ExpressionType.BOOLEAN);
            default:
                throw new SymbolicException.NotImplemented("Method toBoolean not implemented for type '" + expression.getRight().toString() + "'.");
        }
    }
    ...
    public static final Pair<Expr, ExpressionType> negateNumericZ3JS(Context ctx, Pair<Expr, ExpressionType> expression) throws SymbolicException.UndecidableExpression { //TODO
        switch (expression.getRight()) {
            ...
            default:
                throw new SymbolicException.UndecidableExpression("Z3", "Cannot negate expression with type '" + expression.getRight().toString() + "'.");
        }
    }
    ...
    public static boolean checkTypeIsNumeric(Pair<Expr, ExpressionType>... parameters) {
        for (Pair<Expr, ExpressionType> param : parameters) {
            if (param.getRight() == ExpressionType.BOOLEAN
                    || param.getRight() == ExpressionType.STRING
                    || param.getRight() == ExpressionType.UNDEFINED
                    || param.getRight() == ExpressionType.NULL
                    || param.getRight() == ExpressionType.OBJECT
                    || param.getRight() == ExpressionType.SYMBOL) {
                return false;
            }
        }
        return true;
     ...
     */
    /*public boolean allChildrenType(ExpressionType check_type) {
        for (SymbolicNode cn: this.children) {
     ...
     */
    /*public boolean allChildrenTypeOr(ExpressionType... check_type) {
        for (SymbolicNode cn: this.children) {
            boolean found = false;
            for (ExpressionType t: check_type) {
                if (cn.type == t) {
                    found = true;
                    break;
                }
