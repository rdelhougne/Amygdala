...
public class Tracer {
	...
	private final HashMap<Integer, SymbolicNode> interim_results = new HashMap<>();

	// Saves all results of the symbolic stack frame
	private final HashMap<String, SymbolicNode> symbolic_frame = new HashMap<>();
	...
	public void read_frame_to_interim(String variable_name, Integer to_node) {
		if (symbolic_frame.containsKey(variable_name)) {
			...
			logger.critical("Tracer::read_frame_to_interim(): Symbolic frame does not contain key '" +
                                    variable_name + "'");
		}
	}
	 ...
	 */
	public void add_operation(Integer node_target, LanguageSemantic s, Operation op, Integer node_source_a,
		...
		if (interim_results.containsKey(node_source_a) && interim_results.containsKey(node_source_b)) {
			...
			if (!interim_results.containsKey(node_source_a) && !interim_results.containsKey(node_source_b)) {
				logger.critical("Tracer::add_operation(): Trying to add operation " + op.toString() +
										" but interim result from " + node_source_a + " and " + node_source_b +
										" does not exist");
			} else if (!interim_results.containsKey(node_source_a)) {
				logger.critical("Tracer::add_operation(): Trying to add operation " + op.toString() +
										" but interim result from " + node_source_a + " does not exist");
			} else {
				logger.critical("Tracer::add_operation(): Trying to add operation " + op.toString() +
										" but interim result from " + node_source_b + " does not exist");
			}
		}
	}

	/**
	 * This is an overloaded method of {@link #add_operation(Integer, LanguageSemantic, Operation, Integer, Integer)}
	 ...
	 * @throws SymbolicException.WrongParameterSize
	 */
	public void add_operation(Integer node_target, LanguageSemantic s, Operation op, Integer node_source) throws
			SymbolicException.WrongParameterSize {
		if (interim_results.containsKey(node_source)) {
			...
			logger.critical("Tracer::add_operation(): Trying to add operation " + op.toString() +
                            " but interim result from " + node_source + " does not exist");
		}
	}
	...
	public void initialize_program_context(LanguageSemantic sem) {
		if (sem == LanguageSemantic.JAVASCRIPT) {
			...
			logger.warning("Tracer::initialize_program_context(): Cannot initialize context with semantic '" +
                                   sem.toString() + "'");
		}
	}
}
