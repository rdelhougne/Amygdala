...
const IN_Basal = 1;
const IN_IDLE = 1;
const IN_INFUSION_MANAGER = 1;
const IN_INTERMITTENT_BOLUS = 2;
const IN_LOCKOUT = 1;
const IN_MANUAL_PAUSED_KVO = 1;
const IN_NOT_ON = 2;
const IN_NO_ACTIVE_CHILD = 0;
const IN_OFF = 1;
const IN_OFF_B = 2;
const IN_ON = 2;
const IN_ON_B = 3;
const IN_PAUSED = 2;
const IN_Patient_Bolus = 3;
const IN_PAUSED_KVO = 2;
const IN_PAUSED_NoKVO = 3;
const IN_THERAPY = 2;

class InfusionMgrFunctional {
  stepScalingFactor(inputVal) {
  ...
  sbolus_trigger(localB, localDW) {
    ...
    scalingFactor = this.stepScalingFactor(localB.intervalIntermittentBolus);
    sbolusInterTimer_l = localDW.sbolusInterTimer;

    if (!((sbolusInterTimer_l > scalingFactor || (sbolusInterTimer_l < scalingFactor))
        || (!(sbolusInterTimer_l == scalingFactor)))) {
      /* Transition: '<S1>:3949' */
      sb = 1;
      localDW.sbolusInterTimer = 0;
    } else {
  ...
  enterInternalActive(localB, localDW) {
    ...
    localDW.sbolusTimer = 0;
    localDW.sbolusReq = false;
    localDW.sbolusReq = (this.sbolus_trigger(localB, localDW) != 0);
  ...
  enterInternalPaused(localB, localDW) {
    ...
    /* Transition: '<S1>:3881' */
    isAlarmPaused_l = localDW.isAlarmPaused;
    highestLevelAlarm_l = localB.highestLevelAlarm;
  ...
  resetAllInfusionDetails(localB, localDW) {
    ...
    /* Transition: '<S1>:3961' */
    this.resetForNewInfusion(localB, localDW);
    localDW.lockTimer = 0;
    localDW.inPatientBolus = false;
    localDW.numberPbolus = 0;
  }
  ...
  therapy(localB, localDW) {
    ...
    if ((infusioninitiateL && reservoiremptyL) || (configuredL < 1) || infusionCancelL) {
      ...
      if (infusionInhibitL || (highestLevelAlarm_l >= 3)) {
        ...
        this.enterInternalPaused(localB, localDW);
      } else {
        /* Transition: '<S1>:3875' */
        localDW.isTherapy = IN_ACTIVE;
        this.enterInternalActive(localB, localDW);
      }
    } else {
      ...
      var scalingFactor = this.stepScalingFactor(localB.infusionTotalDuration - 1);
        ...
        if (localDW.isTherapy == IN_ACTIVE) {
          ...
          if (infusionInhibitL || (highestLevelAlarmL >= 3)) {
            ...
            if (localDW.isBasal == IN_OFF) {
              ...
              /* During 'ON': '<S1>:3911' */
              scalingFactor = this.stepScalingFactor(localB.infusionTotalDuration - 1);
            ...
            if (localDW.isPatient == IN_LOCKOUT) {
              ...
              scalingFactor = this.stepScalingFactor((int) (localB.lockoutPeriodPatientBolus - 1));
              ...
              /* During 'ON': '<S1>:3934' */
              scalingFactor = this.stepScalingFactor(localB.durationPatientBolus - 1);
            ...
            if (localDW.isIntermittent == IN_OFF) {
              ...
              /* During 'ON': '<S1>:3941' */
              scalingFactor = this.stepScalingFactor(localB.durationIntermittentBolus - 1);
